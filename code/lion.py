...
def setup_logging(optimizer_name='LION'):
...
class Lion(torch.optim.Optimizer):
    """LION optimizer implementation based on the paper
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # LION update rule
                update = exp_avg.sign().add_(p, alpha=group['weight_decay'])
                p.add_(update, alpha=-group['lr'])
        
        return loss

# Training Function for LION
def train_model(model, train_loader, val_loader, optimizer_name='LION', num_epochs=50, lr=1e-4, betas=(0.9, 0.99)):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Evolved Sign Momentum)")
    model = model.to(device)
    
    criterion = nn.CrossEntropyLoss()
    
    # Use LION optimizer
    optimizer = Lion(model.parameters(), lr=lr, betas=betas, weight_decay=1e-2)
    ...
    logging.info(f"Learning rate: {lr}, Betas: {betas}, Weight decay: 1e-2")
    logging.info(f"Batch size: {train_loader.batch_size}")
    logging.info(f"LION uses sign-based updates and momentum for efficient optimization")
    logging.info(f"LION typically uses lower learning rates and higher weight decay")
    
    for epoch in range(num_epochs):
        model.train()
        epoch_loss = 0.0
        
        # Mini-batch training with LION
        ...
        for batch_idx, (images, masks, _, _) in enumerate(train_bar):
            ...
            # Backward pass and optimize with LION
            loss.backward()
            optimizer.step()
...
# Testing Function
def test_model(model, test_loader, optimizer_name='LION'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # Use mini-batch training for LION
    ...
    train_losses, val_losses, val_ious, val_accuracies = train_model(
        ...
        num_epochs=50, lr=1e-4, betas=(0.9, 0.99)
    )
