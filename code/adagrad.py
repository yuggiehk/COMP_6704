...
def setup_logging(optimizer_name='AdaGrad'):
...
# Training Function for AdaGrad
def train_model(model, train_loader, val_loader, optimizer_name='AdaGrad', num_epochs=50, lr=0.01):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Adaptive Gradient Algorithm)")
    ...
    logging.info(f"Learning rate: {lr}, Weight decay: 1e-4")
    logging.info(f"Batch size: {train_loader.batch_size}")
    logging.info(f"AdaGrad adapts learning rates per parameter based on historical gradients")
    logging.info(f"Note: Higher initial learning rate is often used for AdaGrad")
    
    for epoch in range(num_epochs):
        model.train()
        epoch_loss = 0.0
        
        # Mini-batch training with AdaGrad
        ...
        for batch_idx, (images, masks, _, _) in enumerate(train_bar):
            ...
            # Backward pass and optimize with AdaGrad
            loss.backward()
            optimizer.step()
...
# Testing Function
def test_model(model, test_loader, optimizer_name='AdaGrad'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # Use mini-batch training for AdaGrad
    ...
    train_losses, val_losses, val_ious, val_accuracies = train_model(
        ...
        num_epochs=50, lr=0.01  # Higher learning rate for AdaGrad
    )
