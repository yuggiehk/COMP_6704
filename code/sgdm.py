...
def setup_logging(optimizer_name='SGDM'):
...
# Training Function for Stochastic Gradient Descent with Momentum (SGDM)
def train_model(model, train_loader, val_loader, optimizer_name='SGDM', num_epochs=50, lr=0.001, momentum=0.9):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Stochastic Gradient Descent with Momentum)")
    ...
    logging.info(f"Learning rate: {lr}, Momentum: {momentum}, Weight decay: 1e-4")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, masks, _, _) in enumerate(train_bar):
            ...
            # Backward pass and optimize with momentum
            loss.backward()
            optimizer.step()
...
# Testing Function
def test_model(model, test_loader, optimizer_name='SGDM'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # Use mini-batch training for SGDM
