...
def setup_logging(optimizer_name='MiniBatch_GD'):
...
# Training Function for Mini-Batch Gradient Descent
def train_model(model, train_loader, val_loader, optimizer_name='MiniBatch_GD', num_epochs=50, lr=0.001):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Mini-Batch Gradient Descent)")
    model = model.to(device)
    
    criterion = nn.CrossEntropyLoss()
    
    # Use basic Gradient Descent optimizer (no momentum) for mini-batch
    ...
    logging.info(f"Using mini-batch gradient descent (batch size = {train_loader.batch_size})")
    
    for epoch in range(num_epochs):
        model.train()
        epoch_loss = 0.0
        
        # Mini-batch training - update weights for each batch
        train_bar = tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs} [{optimizer_name}-Train]')
        for batch_idx, (images, masks, _, _) in enumerate(train_bar):
...
# Testing Function
def test_model(model, test_loader, optimizer_name='MiniBatch_GD'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # For mini-batch GD, we use smaller batch size
    ...
    plt.title(f'Training Loss ({optimizer_name} Optimizer - Mini-Batch)', fontsize=16, fontweight='bold')
    ...
    plt.title(f'Validation Loss ({optimizer_name} Optimizer - Mini-Batch)', fontsize=16, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Loss', fontsize=12)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.title(f'Validation IoU ({optimizer_name} Optimizer - Mini-Batch)', fontsize=16, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12)
    ...
    plt.title(f'Validation Accuracy ({optimizer_name} Optimizer - Mini-Batch)', fontsize=16, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12)
    ...
    logging.info("\n" + "="*80)
    logging.info(f"Final Training Results ({optimizer_name} Optimizer - Mini-Batch):")
    logging.info("="*80)
    ...
    logging.info(f"\nTraining with {optimizer_name} optimizer (Mini-Batch) completed! All results saved.")
