...
def setup_logging(optimizer_name='RMSProp'):
...
# Training Function for RMSProp
def train_model(model, train_loader, val_loader, optimizer_name='RMSProp', num_epochs=50, lr=0.001, alpha=0.99):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Root Mean Square Propagation)")
    model = model.to(device)
    
    criterion = nn.CrossEntropyLoss()
    
    # Use RMSProp optimizer
    optimizer = optim.RMSprop(model.parameters(), lr=lr, alpha=alpha, weight_decay=1e-4)
    ...
    logging.info(f"Learning rate: {lr}, Alpha: {alpha}, Weight decay: 1e-4")
    logging.info(f"Batch size: {train_loader.batch_size}")
    logging.info(f"RMSProp uses moving average of squared gradients to adapt learning rates")
    logging.info(f"Alpha parameter controls the smoothing factor for gradient history")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, masks, _, _) in enumerate(train_bar):
            ...
            # Backward pass and optimize with RMSProp
            loss.backward()
            optimizer.step()
...
# Testing Function
def test_model(model, test_loader, optimizer_name='RMSProp'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # Use mini-batch training for RMSProp
    ...
    train_losses, val_losses, val_ious, val_accuracies = train_model(
        ...
        num_epochs=50, lr=0.001, alpha=0.99
    )
