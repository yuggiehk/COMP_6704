...
def setup_logging(optimizer_name='AdamW'):
...
# Training Function
def train_model(model, train_loader, val_loader, optimizer_name='AdamW', num_epochs=50, lr=0.001):
...
# Testing Function
def test_model(model, test_loader, optimizer_name='AdamW'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
