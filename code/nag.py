...
def setup_logging(optimizer_name='NAG'):
...
# Training Function for Nesterov Accelerated Gradient (NAG)
def train_model(model, train_loader, val_loader, optimizer_name='NAG', num_epochs=50, lr=0.001, momentum=0.9):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Nesterov Accelerated Gradient)")
    model = model.to(device)
    
    criterion = nn.CrossEntropyLoss()
    
    # Use SGD with Nesterov momentum (NAG)
    ...
    logging.info(f"Learning rate: {lr}, Momentum: {momentum}, Nesterov: True, Weight decay: 1e-4")
    logging.info(f"Batch size: {train_loader.batch_size}")
    logging.info(f"NAG uses lookahead gradient calculation for better convergence")
    
    for epoch in range(num_epochs):
        model.train()
        epoch_loss = 0.0
        
        # Mini-batch training with Nesterov momentum
        ...
        for batch_idx, (images, masks, _, _) in enumerate(train_bar):
            ...
            # Backward pass and optimize with Nesterov momentum
            loss.backward()
            optimizer.step()
...
# Testing Function
def test_model(model, test_loader, optimizer_name='NAG'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # Use mini-batch training for NAG
