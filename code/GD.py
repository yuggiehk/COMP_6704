...
def setup_logging(optimizer_name='GD'):
...
# Training Function for Gradient Descent (using full dataset)
def train_model(model, train_loader, val_loader, optimizer_name='GD', num_epochs=50, lr=0.001):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    logging.info(f"Training with {optimizer_name} optimizer (Full Batch Gradient Descent)")
    model = model.to(device)
    
    criterion = nn.CrossEntropyLoss()
    
    # Use basic Gradient Descent optimizer (no momentum)
    optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.0, weight_decay=1e-4)
    ...
    logging.info(f"Learning rate: {lr}, Momentum: 0.0, Weight decay: 1e-4")
    logging.info(f"Using full batch gradient descent (batch size = entire dataset)")
    
    for epoch in range(num_epochs):
        model.train()
        train_loss = 0.0
        
        # For full batch GD, we need to accumulate gradients over the entire dataset
        optimizer.zero_grad()
        epoch_loss = 0.0
        
        # Process the entire dataset to compute full gradient
        ...
        for images, masks, _, _ in train_bar:
            ...
            batch_loss = loss * images.size(0) / len(train_loader.dataset)
            batch_loss.backward()
            
            epoch_loss += loss.item() * images.size(0)
            train_bar.set_postfix(loss=loss.item())
        
        # Update weights using the accumulated gradient (full batch)
        optimizer.step()
...
# Testing Function
def test_model(model, test_loader, optimizer_name='GD'):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    model.eval()
...
def main():
    ...
    # For full batch GD, we use batch_size = entire dataset
    ...
    # Use GD optimizer (Full Batch Gradient Descent)
    logging.info(f"\n{'='*60}")
    logging.info(f"Training with {optimizer_name} optimizer (Full Batch Gradient Descent)")
    logging.info(f"{'='*60}")
    ...
    plt.title(f'Training Loss ({optimizer_name} Optimizer - Full Batch)', fontsize=16, fontweight='bold')
    ...
    plt.title(f'Validation Loss ({optimizer_name} Optimizer - Full Batch)', fontsize=16, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Loss', fontsize=12)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.title(f'Validation IoU ({optimizer_name} Optimizer - Full Batch)', fontsize=16, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12)
    ...
    plt.title(f'Validation Accuracy ({optimizer_name} Optimizer - Full Batch)', fontsize=16, fontweight='bold')
    plt.xlabel('Epoch', fontsize=12)
    ...
    logging.info("\n" + "="*80)
    logging.info(f"Final Training Results ({optimizer_name} Optimizer - Full Batch):")
    logging.info("="*80)
    ...
    logging.info(f"\nTraining with {optimizer_name} optimizer (Full Batch) completed! All results saved.")
